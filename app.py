...
<style>
    ...
    div[data-testid="stFileUploader"] {
        background: #1a2332; border: 2px dashed #2a4a6a;
    ...
    .stButton > button {
        ...
        font-family: 'IBM Plex Sans'; width: 100%;
        transition: all 0.2s;
    }
...
def get_page_count(pdf_bytes: bytes) -> int:
    """Fast page count — opens doc without rendering anything."""
...
def render_page(pdf_bytes: bytes, page_index: int, dpi: int = 150) -> bytes:
    """
    Render ONE page on demand and cache it.
    DPI 150 is enough for Gemini vision — 2x (288dpi) was overkill and slow.
    """
    doc  = fitz.open(stream=pdf_bytes, filetype="pdf")
    page = doc.load_page(page_index)

    # Adaptive DPI — avoid malloc on large pages
    rect = page.rect
    max_px = 3_000_000
    scale  = dpi / 72
...
def render_result(data: dict):
    ...
    <div class="{card_cls}">
        ...
        <div style="color:#8899aa;font-size:0.88rem">
            {"Proposed grade is BELOW existing ground — excavation needed"
              if cf_type == "CUT" else
             "Proposed grade is ABOVE existing ground — fill material needed"
              if cf_type == "FILL" else
             "Lines at approximately the same elevation"}
        </div>
        <div class="info-grid">
            <div class="info-item"><div class="k">Station</div>
                <div class="v">{data.get("station") or "—"}</div></div>
            <div class="info-item"><div class="k">Elevation</div>
                <div class="v">{data.get("elevation") or "—"}</div></div>
            <div class="info-item"><div class="k">Slopes</div>
                <div class="v">{data.get("slopes") or "—"}</div></div>
            <div class="info-item"><div class="k">Intersections</div>
                <div class="v">{data.get("intersection_points") or "—"}</div></div>
            <div class="info-item"><div class="k">Area</div>
                <div class="v">{data.get("Area") or "—"}</div></div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with c1:
        st.markdown(f"""<div class="info-item">
            <div class="k">Existing Ground</div>
            <div class="v">{data.get("existing_ground_line","—")}</div></div>""",
            unsafe_allow_html=True)
    with c2:
        st.markdown(f"""<div class="info-item">
...
with col_left:
    ...
    # Render only the selected page (cached)
...
with col_right:
    ...
    if analyze_clicked:
        ...
        st.markdown("""
        <div style="text-align:center;padding:4rem 2rem;color:#4a6080;border:1px dashed #2a3f5a;border-radius:10px;">
            <div style="font-size:2.5rem">📊</div>
            <p style="margin-top:0.8rem;font-size:0.9rem">
                Select a page and click<br><strong style="color:#7eb8f7">Analyze This Page</strong>
            </p>
        </div>
        """, unsafe_allow_html=True)